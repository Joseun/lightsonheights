...
def clean_text_data(df):
    ...
    search_str = r'http[s]?://(?:[a-zA-Z]|[0-9]|[$-_@.&+]|[!*\(\),]|(?:%[0-9a-fA-F][0-9a-fA-F]))+'
    text_data['corpus'] = text_data['corpus'].str.replace(search_str, '')
...
def run_data_prep(raw_data_path: str, dest_path: str):
    ...
    dump_pickle(vectorizer, os.path.join(dest_path, "vectorizer.pkl"))
    dump_pickle((X_train, y_train), os.path.join(dest_path, "train.pkl"))
    dump_pickle((X_val, y_val), os.path.join(dest_path, "val.pkl"))

    mlflow.log_artifact(
        os.path.join(args.dest_path, "vectorizer.pkl"), artifact_path="vectorizer"
    )
